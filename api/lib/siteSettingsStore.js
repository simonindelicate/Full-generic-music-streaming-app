...
const getFilePath = () =>
  process.env.SITE_SETTINGS_PATH ||
  path.join(config.storageRoot, 'metadata', 'site-settings.json');
...
async function loadSiteSettings({ bypassCache = false } = {}) {
  ...
  const settings = await readFileStore();
...
async function saveSiteSettings(payload) {
  ...
  const result = await writeFileStore(settings);
  memoryCache = settings;
