...
const JSON_TRACKS_PATH = process.env.TRACKS_JSON_PATH || path.join('/tmp', 'tracks.json');
...
exports.handler = async (event) => {
  try {
    ...
    if (preferredStore === 'mongodb') {
      ...
      return jsonResponse(200, { message: 'Tracks added!', store: 'mongodb' });
    }
    ...
    return jsonResponse(200, {
      ...
      path: JSON_TRACKS_PATH,
      note: "JSON storage on serverless is ephemeral unless TRACKS_JSON_PATH points to persistent storage.",
    });
  } catch (err) {
    console.error(err);
